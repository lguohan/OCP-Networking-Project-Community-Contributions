 ...
 *   buffer for port_list and set the port_count to the allocated buffer size. If
 *   the size is large enough to accomodate the list of port id, the callee will
 *   then fill the port_list and set the port_count to the actual number of ports.
 *   If the buffer is not large enough, the callee will set the port_count
 *   to the actual number of port id and return SAI_STATUS_BUFFER_OVERFLOW. Once
 *   the caller gets such return code, it should use the returned port count to 
 *   allocate enough buffer and retry.
 */
